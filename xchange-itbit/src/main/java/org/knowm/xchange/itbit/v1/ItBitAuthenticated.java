...
public interface ItBitAuthenticated {
  ...
  ItBitOrder getOrder(@HeaderParam("Authorization") ParamsDigest signer, @HeaderParam("X-Auth-Timestamp") long timestamp,
                      ...
                      @PathParam("orderId") String orderId) throws IOException, ItBitException;
  ...
  ItBitWithdrawalResponse requestWithdrawal(@HeaderParam("Authorization") ParamsDigest signer, @HeaderParam("X-Auth-Timestamp") long timestamp,
                                            @HeaderParam("X-Auth-Nonce") SynchronizedValueFactory<Long> valueFactory, @PathParam("walletId") String walletId,
                                            ItBitWithdrawalRequest request) throws IOException, ItBitException;
  ...
  @Path("wallets")
  @Consumes(MediaType.APPLICATION_JSON)
  List wallets(@HeaderParam("Authorization") ParamsDigest signatureCreator, @HeaderParam("X-Auth-Nonce") SynchronizedValueFactory<Long> nonceFactory, @HeaderParam("X-Auth-Timestamp") long timestamp,
               @QueryParam("userId") String userId, @QueryParam("page") int page, @QueryParam("perPage") int perPage);

  @GET
  @Path("wallets/{walletId}/funding_history")
  @Consumes(MediaType.APPLICATION_JSON)
  ItBitFundingHistoryResponse fundingHistory(@HeaderParam("Authorization") ParamsDigest signatureCreator, @HeaderParam("X-Auth-Nonce") SynchronizedValueFactory<Long> nonceFactory, @HeaderParam("X-Auth-Timestamp") long timestamp,
                                             @PathParam("walletId") String walletId, @QueryParam("page") int page, @QueryParam("perPage") int perPage);
}
